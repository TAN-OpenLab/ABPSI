...
Server::Server(int num_xpoints, int dbs_size, int pub_mod_bitsize, int maxSetsize, int NoEl_bucket, int tb_size){
	...
	//ning
	ck_server = rd.gen_randSet(1, 20); // 20 is an arbitrary choioce.
	//ning
...
//**********************************************************************
//Function description: computes the result, given all clients messages and their outsourced data.
Server_Result * Server::compute_result_ning (GrantComp_Info** grantComp_info, byte* tmp_key_, byte *tmp_iv_){
	...
	o_all = (mpz_t**)malloc(table_size * sizeof(mpz_t));
	


	for(int i = 0;i < table_size; i++){
		o_all[i] = (mpz_t*)malloc((NoElem_in_bucket+1) * sizeof(mpz_t));
		for(int j = 0;j < NoElem_in_bucket+1; j++){
			mpz_init(o_all[i][j]);
			mpz_set(o_all[i][j],zero);
		}
	}
	...
	for (int t = 0; t < db_size - 1; t++){//db_size : number of clients
		for(int i = 0;i < table_size; i++){
			 ...
			 o_A_ning = db[t].poly[i].get_blind_coefficients();

			for(int j = 0;j < NoElem_in_bucket+1; j++){
				mpz_add(o_all[i][j],o_all[i][j],o_A_ning[j]);
				mpz_mod(o_all[i][j],o_all[i][j],pu_moduli[0]);
			}
	...
	for(int i = 0;i < table_size; i++){
			for(int j = 0;j < NoElem_in_bucket+1; j++){
				mpz_add(o_all[i][j],o_all[i][j],ck_server[0]);
				mpz_mod(o_all[i][j],o_all[i][j],pu_moduli[0]);
			}

		}
	ptr->o_all_result = o_all;

	bigint * BF_temp;
	bigint * BF_all;
	BF_temp = (mpz_t*)malloc(table_size * sizeof(mpz_t));
	BF_all = (mpz_t*)malloc(table_size * sizeof(mpz_t));
	for(int i = 0;i < table_size; i++){
		
			mpz_init(BF_temp[i]);
			mpz_set(BF_temp[i],zero);

			mpz_init(BF_all[i]);
			mpz_set(BF_all[i],zero);
		
	}
	bigint **BF_temp2;
	BF_temp2 = (mpz_t**)malloc(db_size * sizeof(mpz_t));
	for (int t = 0; t < db_size - 1; t++){
		BF_temp2[t] = db[t].BF_ning;
		BF_temp2[t] = (mpz_t*)malloc(table_size * sizeof(mpz_t));
	...
	for(int i = 0;i < table_size; i++){
		...
		for (int t = 0; t < db_size - 1; t++){//db_size : number of clients
			...
			mpz_set(BF_temp[i],BF_temp2[t][i]);
			mpz_and(BF_all[i], BF_all[i], BF_temp[i]);
		}
	}
	 ptr->BF_all_ning = BF_all;
	//ninged
