...
Client::Client(Server*server, bigint *elements, int el_size){
	...
	//bk_ning = rd_.gen_randSet(1,5);
	bk_ning = (mpz_t*)malloc(1 * sizeof(mpz_t));
	bigint bk_set;
	
	mpz_init_set_str(bk_set, "1471309", 10);
	mpz_init_set(bk_ning[0],bk_set);
...
//**********************************************************************
//ning
// - Function description: returns bk ning.
bigint*  Client::get_bk_ning(){
...
void Client::set_tree_q_A_ning_no_child(){
	tree_q_A_ning =q_ning;
}


unsigned char  Client::get_bloomning(){

	unsigned char ptr1 = bloomtable_ning;

	return ptr1;

}
int  Client::get_something(){
...
bigint*  Client::PR_shuffle_ning(bigint* elem, int size, bigint seed){
	
	int  indx, buf2;
	int shuffle_map[size];

	indx = mpz_get_ui(seed)%size;
	bigint *res;
	...
	for (int  j = size - 1; j > 0; j--){
		...
		indx = (key_A*indx+key_B)%size;
		//exchange
		buf2 = shuffle_map[j];
		shuffle_map[j] = shuffle_map[indx];
		shuffle_map[indx] = buf2;
	
	}
	for(int i = 0; i < size; i++){
		mpz_set(elem[i],res[shuffle_map[i]]);
	}
	return elem;
}
//**********************************************************************
...
int**  Client::PR_shuffle_ning(int** elem, int size, bigint seed){
...
Polynomial*  Client::PR_shuffle_poly_ning(Polynomial* pol, int size, bigint seed){
...
//ningbg
// - Function description: converts a bloom filter to a biginteger value.
bloom_filter Client::shuffle_one_BF(bloom_filter filter){
	
	int size = filter.bit_table_.size();
	int  indx;
	int shuffle_map[size];

	indx = mpz_get_ui(bk_ning[0])%size;
	unsigned char ar[size], res[size];
	unsigned char buf1;
	int buf2;
	for(int i = 0; i < size; i++){
		ar[i] = filter.bit_table_[i];
	...
	for(int i = 0; i < size; i++){
		filter.bit_table_[i] = ar[shuffle_map[i]];
	}
	return filter;
}
//ning额的
...
//**********************************************************************
// - Function description: converts a bloom filter to a biginteger value.
int* Client::convert_BF_to_int_ning(bloom_filter filter){
	...
	for(int i = 0; i < size; i++){
		...
		//ning
		for(int j=7;j>=0;j--){
			bf_ning[i*8+7-j] = ((ar[i] >> j) & 0x01);
		}
			//int * shuffledbf;

			//res=PR_shuffle_ning(bf_ning, table_size, bk[0]);

		//ning
	}

	bloomtable_ning = ar[3];//ning3
	//bloomtable_ning = 0x11011;//ning3
...
//**********************************************************************
// - Function description: converts a bloom filter to a biginteger value.
bigint* Client::convert_BF_to_char_ning(bloom_filter filter){
	...
	bloomtable_ning = filter.bit_table_[0];//ning3
...
//**********************************************************************
// - Function description: given a hashtable containing set elements, it assigns a bloom filter to each bin of the table.
bigint* Client::assing_BFs2HT_ning_bigint(Hashtable HT, int NoElem_in_bucket, int table_size, bloom_parameters parameters){
	...
	for(int i = 0; i < table_size; i++){
		...
		//shuffle one bloom filter begin
		filter = shuffle_one_BF(filter);
...
//ningbg
// - Function description: given a hashtable containing set elements, it assigns a bloom filter to each bin of the table.
int** Client::assing_BFs2HT_ning(Hashtable HT, int NoElem_in_bucket, int table_size, bloom_parameters parameters){
	...
	int *bfrow;
	//int size;
	int bfsize = filter.bit_table_.size();
	bfsize_ning = bfsize;
	int** bfmatrix;
	bfmatrix = new int*[table_size];   //建立包含row个指针的指针数组
	for (int i = 0; i < table_size; i++)
	{
		*(bfmatrix + i) = new int[bfsize];   //每个一维指针数组的长度都是
	}
	bfmatrix[0][0]=1;
	Random rd;//ning
	// retrives the set elements of each bin, and inserts them to bloom filters.
	for(int i = 0; i < table_size; i++){
		...
		for(int j = 0; j < NoElem_in_bucket; j++){
			...
			//ning
				...
				bigint* temp_= rd.get_nonconflict_randset_ning(1, pubmoduli, pub_moduli_bitsize);
				mpz_init_set(temp_ar[j], temp_[0]);
				string s = mpz_get_str(NULL, 10, temp_ar[j]); // this is done to make query and insertion compatible.
				filter.insert(s);
				s.clear();
			}
		...
		bfrow = convert_BF_to_int_ning(filter); 
		//size = sizeof(bfrow)/sizeof(bfrow[0]);


		for (int k=0;k<bfsize;k++){
			bfmatrix[i][k] = bfrow[k];
...
//**********************************************************************
// - Function description: prepare the set elements and sends a blinded dataset to the server.
void Client::outsource_db_ning(string& poly_ID){
	...
	if(poly_ID == "B_ID"){ // this is done only for test-- so it can be tested on devices with small memory-- it can be commented out.
    	bigint_BF = assing_BFs2HT(HT, NoElem_in_bucket, table_size, bf_parameters); // assigns a bloom filters to each bin. It returns an array of bigint representing bloom filters.
    	blinded_BF = blind_BFs_(bigint_BF, table_size , BF_key_, BF_iv, pr_moduli);
    	db.BF = PR_shuffle(bigint_BF, table_size, bk_ning[0]); // permutes the array of bigintegers and stores the result in Client_Dataset that will be sent to the server.
		//db.BF = 
	}

	//ningbg
	bigint_BF = assing_BFs2HT_ning_bigint(HT, NoElem_in_bucket, table_size, bf_parameters);
	db.BF_ning = PR_shuffle_ning(bigint_BF, table_size, bk_ning[0]);
	//ninged
	// //ning
	// bfmatrix = assing_BFs2HT_ning(HT, NoElem_in_bucket, table_size, bf_parameters); // assigns a bloom filters to each bin. It returns an array of bigint representing bloom filters.
    // db.bfmatrix_shuffle = PR_shuffle_ning(bfmatrix, table_size, bk_ning[0]); // permutes the array of bigintegers and stores the result in Client_Dataset that will be sent to the server.
	// //ning-end
	...
	//ningbg
	blind_coefficients = (mpz_t**)malloc(table_size * sizeof(mpz_t));
	keys_blind_coefficients = (mpz_t**)malloc(table_size * sizeof(mpz_t));
	bigint* pol_blind_coef;
  	...
  	for(int i = 0; i < table_size; i++){
		...
		poly[i].blind_poly_ning(seed_, iv, AES::DEFAULT_KEYLENGTH, i, counter[i], pub_moduli_bitsize / 8, pubmoduli);
	
		// //ningbg
		
		mpz_init_set_str(minus_one, "-1", 10);

		pol_blind_coef = poly[i].get_blind_coefficients();
		blind_coefficients[i] = poly[i].get_blind_coefficients();
		blind_coefficients[i] = (mpz_t*)malloc((elem_size+1) * sizeof(mpz_t));
		pol_blind_coef = (mpz_t*)malloc((elem_size+1) * sizeof(mpz_t));

		for(int j=0;j<(elem_size+1);j++){
		
			mpz_set(blind_coefficients[i][j] , pol_blind_coef[j]);
		}
		keys_blind_coefficients[i] = poly[i].get_keys_blind_coefficients();

		//ninged
	}
	db.poly = PR_shuffle_poly_ning(poly, table_size, shuffle_key);
	db.client_ID = poly_ID;
	serv->store_poly(db);
  	delete[] poly;
  	free(labels);
...
//**********************************************************************
bigint** Client::get_blind_coefficients(){
	bigint** res = blind_coefficients;
	return res;
}
bigint** Client::get_keys_blind_coefficients(){
	bigint** res = keys_blind_coefficients;
	return res;
}
...
vector <string> Client::find_intersection_ning(bigint** q_all, Server_Result* res, int*& size, bigint*** Q, int number_of_clients, bigint*** q_ning){
	...
	bigint **o_all;
	o_all = res->o_all_result;
	bigint *bf_res;
	bf_res = res->BF_all_ning;
	for(int i = 0;i < table_size; i++){
		//if( mpz_cmp(bf_res[i],zero)!= 0){
			count_ning = count_ning +1; //ning
			for(int j = 0;j < NoElem_in_bucket+1; j++){
				mpz_add(q_all[i][j],q_all[i][j],o_all[i][j]);
				mpz_mod(q_all[i][j],q_all[i][j],pubmoduli);
			}
			roots = findroots(q_all[i], NoElem_in_bucket+1, number_of_roots, pubmoduli, P); 
		//}
	}
	for(int i = 0;i < table_size; i++){ //0.19
		roots = findroots(q_all[i], NoElem_in_bucket+1, number_of_roots, pubmoduli, P); 
		if(number_of_roots != 0){
			valid_roots = (mpz_t*)malloc(number_of_roots * sizeof(mpz_t));
        	mpz_init_set(valid_roots[0], minus_one);
        	valid_roots = check_vals_in_BF(roots, number_of_roots, bf_res[i], bf_parameters, num_of_elements_found); // extracts the valid roots.
        	if(mpz_cmp(valid_roots[0], minus_one)!= 0 && num_of_elements_found > 0){
				for(int k = 0; k < num_of_elements_found; k++){ // stores the valid roots.
					tempstr.clear();
            		tempstr = mpz_get_str(NULL, 10, valid_roots[k]);
...
//**********************************************************************
//ningbg
bigint ** Client::find_q_all_ning(Server_Result* res, int*& size, bigint*** Q, int number_of_clients, bigint*** q_ning){
	...
	q_all = (mpz_t**)malloc(table_size * sizeof(mpz_t));
	for(int i = 0;i < table_size; i++){
		q_all[i] = (mpz_t*)malloc((NoElem_in_bucket+1) * sizeof(mpz_t));
		for(int j = 0;j < NoElem_in_bucket+1; j++){
			mpz_init(q_all[i][j]);
			mpz_set(q_all[i][j],zero);
		}
	}
	// o_A_ning = (mpz_t*)malloc((NoElem_in_bucket+1) * sizeof(mpz_t));

	for (int t = 0; t < number_of_clients - 1; t++){//db_size : number of clients
		for(int i = 0;i < table_size; i++){

			 //o_A_ning = A_[j]->poly[i].get_blind_coefficients();

			for(int j = 0;j < NoElem_in_bucket+1; j++){
				mpz_add(q_all[i][j],q_all[i][j],q_ning[t][i][j]);
				mpz_mod(q_all[i][j],q_all[i][j],pubmoduli);
			}
	...
	for(int i = 0;i < table_size; i++){
			for(int j = 0;j < NoElem_in_bucket+1; j++){
				mpz_add(q_all[i][j],q_all[i][j],serv->ck_server[0]);
				mpz_mod(q_all[i][j],q_all[i][j],pubmoduli);
			}
	}
	res_q = q_all;
