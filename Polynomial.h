...
public:
	...
	//ning
	bigint*  evaluate_ning (bigint *,  int, bigint);
	bigint* coefficients, *blind_coefficients, *keys_blind_coefficients;
	bigint* get_coefficients();
	bigint* get_blind_coefficients();
	bigint* get_keys_blind_coefficients();
	void blind_poly_ning(byte *key, byte* iv, int key_size, int indx, int counter_indx, int byte_, bigint pubmod);
	int coef_size;
};
